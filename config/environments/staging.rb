...
puts 'RUNNING IN STAGING!!!!!!!!'
  ...
  config.url_options = { protocol: 'https', host: 'quote-staging.simplybusiness.co.uk', port: nil }

  config.action_mailer.default_url_options = { host: config.url_options[:host], port: config.url_options[:port] }
  config.action_mailer.queued_settings = { delivery_method: (ENVIRON.enabled?('LOAD_TEST') ? :test : :exact_target_multi) }
  ...
  # On BNW add WG LBs external IPs to trusted proxies list to have correct remote_ip
  if ENVIRON.enabled?('DOCKER')
    config.action_dispatch.trusted_proxies =
      ActionDispatch::RemoteIp::TRUSTED_PROXIES +
        %w(34.243.169.55/32 34.250.194.43/32).map{|ip| IPAddr.new(ip) }
  end

  config.remove_lead_url     = "https://quote-staging.simplybusiness.co.uk/backoffice"
